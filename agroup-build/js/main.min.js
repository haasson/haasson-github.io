...
function scrollAnimations() {
  ...
  function ScrollMagicInit() {
    ...
    if (document.documentElement.clientWidth > 1440) {
      ...
      }).setPin("#pinContainer").addTo(controller);
    ...
    var headerPopup = document.querySelector('.header-popap-mini');
    var triggerHeaderPopup = document.querySelector('.service');
    new ScrollMagic.Scene({
      triggerElement: triggerHeaderPopup,
      duration: '100%',
      triggerHook: 1 // reverse: false

    }).setClassToggle(headerPopup, 'popup-active').addTo(controller);
    var quizPopups = document.querySelectorAll('.expert-modal');
    var triggerQuizPopup = document.querySelector('.quiz');
    quizPopups.forEach(function (item) {
      new ScrollMagic.Scene({
        triggerElement: triggerQuizPopup,
        duration: '130%',
        triggerHook: 1 // reverse: false

      }).setClassToggle(item, 'visible') // .addIndicators()
      .addTo(controller);
    });
  }
}
...
function magnificPopupsInit() {
  ...
  if (document.documentElement.clientWidth < 992) {
    $('.conclusion__item_border').magnificPopup({
      type: 'inline',
      preloader: false,
      showCloseBtn: true,
      fixedContentPos: true,
      fixedBgPos: true,
      mainClass: 'custom-mfp custom-mfp-form'
    });
  }
...
function showPopupStandart() {
  ...
  if (document.documentElement.clientWidth > 991) {
    var makePopupVisible = function makePopupVisible(e) {
    ...
    link.addEventListener('mouseenter', function () {
      ...
      document.addEventListener('mousemove', makePopupVisible);
    });
    link.addEventListener('mouseleave', function () {
      popup.style.display = 'none';
      document.removeEventListener('mousemove', makePopupVisible);
    });
  } else {
    popup.classList.add('mfp-hide');
    link.setAttribute('href', '#standart');
  }
}
